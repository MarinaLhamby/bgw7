...
func (h *InvoicesDefault) GetTotalAndCustomerCondition() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		invoices, err := h.sv.GetTotalAndCustomerCondition()
		if err != nil {
			response.Error(w, http.StatusInternalServerError, "error getting total based on customer condition")
			return
		}
		response.JSON(w, http.StatusOK, map[string]any{
			"data": invoices,
